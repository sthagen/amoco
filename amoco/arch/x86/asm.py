...
def write_descriptor(fmap,selector,new_desc):
    ...
    if _ti==bit0:
        ...
        Tbl = segbase(LDTR)
    adr = fmap(Tbl)+offset
    fmap._Mem_write(ptr(adr),new_desc.pack())
...
def save_tss32(fmap,reason):
    ...
    #if reason == 'CALLF':
    #    fmap[mem(segbase(TR),16,disp=old_tss.offset_of("link"))] = fmap(TR)
    ...
    if _e.type == GATE_TASK:
        ...
        if desc is not None and desc.s == DESC_SEGMENT:
            if desc.dpl < internals['ring']:
                ...
                logger.warning("inter-privilege-level-interrupt is not implemented yet...")
            else:
...
def i_LAR(i, fmap):
    ...
    if src._is_cst:
        ...
        if e is not None and e.s == DESC_SEGMENT:
            fmap[zf] = bit1
            fmap[dst[0:16]] = composer(cst(0,8),
                                       cst(e.type,4),
                                       cst(e.s,1),
                                       cst(e.dpl,2),
                                       cst(e.p,1))
            if dst.size>16:
                fmap[dst[20:24]] = composer(cst(e.avl,1),
                                            cst(e.l,1),
                                            cst(e.d,1),
                                            cst(e.g,1))
